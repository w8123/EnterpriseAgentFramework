<template>
  <div class="page-container">
    ...
    <el-alert
      ...
      :closable="false"
      show-icon
      title="能力（Composition）是把「一段稳定的多步业务流程」打包成一个粗粒度可调用的单元"
      description="支持多种形态：子智能体能力（系统提示词 + 工具白名单）；交互式表单组合，结构化收集参数后调用目标 Tool。上层智能体仍像调用普通 Tool 一样选用它们。"
    ...
    <el-card shadow="never">
      ...
      <el-table :data="compositions" v-loading="loading" stripe>
        ...
        <el-table-column label="形态" width="130" align="center">
          <template #default="{ row }">
            <el-tag size="small">{{ formatCompositionKindLabel(row.skillKind) }}</el-tag>
          </template>
        </el-table-column>
      ...
      <el-empty v-if="!loading && compositions.length === 0" description="尚未创建能力，点右上角新建" />
    ...
    >
      <div class="capability-editor">
        <aside class="editor-rail">
          <button
            ...
            type="button"
            class="editor-step"
            :class="{ active: activeCompositionStep === index }"
            @click="activeCompositionStep = index"
          >
        ...
        <section class="editor-main">
          <div class="editor-summary">
            <div>
              <p class="summary-eyebrow">{{ formatCompositionKindLabel(form.skillKind) }}</p>
          ...
          <el-form label-width="112px" class="editor-form">
            ...
            <div v-show="activeCompositionStep === 1" class="editor-panel">
              <div class="panel-heading">
            ...
            <div v-show="activeCompositionStep === 2" class="editor-panel">
              <div class="panel-heading">
                <h4>Agent 调用入口</h4>
      ...
      <template #footer>
        <div class="editor-footer">
          <el-button :disabled="activeCompositionStep === 0" @click="activeCompositionStep -= 1">上一步</el-button>
          <el-button :disabled="activeCompositionStep >= capabilityEditorSteps.length - 1" @click="activeCompositionStep += 1">
            下一步
          </el-button>
        </div>
        ...
        <el-alert v-else :type="testResult.success ? 'success' : 'error'" :closable="false" show-icon>
          ...
          <template #default>
            ...
            <p v-if="isPendingQuotaBlockError(testResult.errorMessage)" style="margin: 8px 0 0">
              ...
              <span class="param-hint">（管理端测试会话 composition-admin-test，最多 5 条未完成）</span>
            </p>
          </template>
        </el-alert>
        <pre
    ...
    >
      <el-alert
        ...
        title="说明"
        description="页面「测试」按钮使用固定测试身份（composition-admin-test，legacy 会话 id）。交互式表单组合挂起后会计入未完成条数，达到 5 条后将无法新开测试。可在此取消单条或全部取消后重试。"
      />
...
const compositions = ref<CompositionInfo[]>([])
...
const activeCompositionStep = ref(0)
/** 打开编辑对话框时该行是否为草稿（控制「暂存」与表单内「启用」） */
const editingIsDraft = ref(false)
const form = reactive<CompositionUpsertRequest>(createEmptyForm())
...
const testingSkill = ref<CompositionInfo | null>(null)
...
const metricsData = ref<CompositionMetrics | null>(null)

const pendingDialogVisible = ref(false)
const pendingList = ref<CompositionAdminTestPendingItem[]>([])
...
function createEmptyForm(): CompositionUpsertRequest {
  return {
    name: '',
    description: '',
    parameters: [],
...
async function fetchCapabilities() {
  loading.value = true
  try {
    const { data } = await listCompositions(buildListParams())
    if (data && 'records' in data) {
      compositions.value = Array.isArray(data.records) ? data.records : []
...
function applyForm(data: CompositionUpsertRequest) {
  form.name = data.name
...
function openEditDialog(skill: CompositionInfo) {
...
function buildUpsertPayload(draft: boolean): CompositionUpsertRequest {
  return {
    ...form,
...
async function handleSaveDraft() {
  ...
  try {
    ...
    if (isEditMode.value && editingName.value) {
      await updateComposition(editingName.value, payload)
...
async function handleDelete(skill: CompositionInfo) {
  try {
    await ElMessageBox.confirm(`确认删除能力 ${skill.name} 吗？`, '删除确认', { type: 'warning' })
    await deleteComposition(skill.name)
...
async function handleEnabledChange(skill: CompositionInfo, enabled: boolean) {
  ...
  try {
    await toggleComposition(skill.name, enabled)
...
function openTest(skill: CompositionInfo) {
  if (skill.draft) {
...
async function handleTest() {
  ...
  try {
    ...
    const { data } = await testComposition(testingSkill.value.name, args)
...
async function handleTestUiAction(action: string, values: Record<string, unknown>) {
  ...
  try {
    const { data } = await testCompositionResume(testingSkill.value.name, {
...
async function openMetrics(skill: CompositionInfo) {
  metricsSkillName.value = skill.name
  metricsData.value = null
  metricsDialogVisible.value = true
  try {
    const { data } = await getCompositionMetrics(skill.name, 7)
