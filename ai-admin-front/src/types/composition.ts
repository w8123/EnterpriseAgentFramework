...
/** 交互式表单组合（Interactive Form Capability）的 spec_json 结构（与后端 InteractiveFormSpec 对齐） */
...
export interface CompositionUpsertRequest {
  name: string
  description: string
  parameters: ToolParameter[]
  skillKind: string
...
export interface CompositionListQuery {
  current?: number
  size?: number
  keyword?: string
  enabled?: boolean
...
export interface CompositionPageResult {
  records: CompositionInfo[]
  total: number
  size: number
  current: number
  pages: number
...
/** 与后端 CompositionController 的测试结果 DTO 对齐 */
...
/** 后端 JSON 仍使用 skillName（legacy）；管理端测试会话 userId=composition-admin-test */
export interface CompositionAdminTestPendingItem {
...
export const COMPOSITION_KIND_LABELS: Record<string, string> = {
  SUB_AGENT: '子智能体能力',
  INTERACTIVE_FORM: '交互式表单组合',
}

export function formatCompositionKindLabel(kind?: string | null): string {
  const k = (kind ?? '').trim()
  if (!k) return COMPOSITION_KIND_LABELS.SUB_AGENT
  return COMPOSITION_KIND_LABELS[k] ?? kind ?? '-'
}
...
export const CAPABILITY_KIND_OPTIONS = [
  ...
  { value: 'INTERACTIVE_FORM', label: '交互式表单组合' },
] as const
