...
Enterprise Agent Framework 是一个面向 **Java 企业系统** 的 AI 能力注册中心与 Agent 治理运行平台。它不是又一个聊天 Demo，而是把企业里已经存在的 Spring Boot / Java 服务、接口、知识库、工具、Skill 和 Agent 统一纳入一个可审批、可观测、可回滚、可对外开放的 AI 中台。
...
- 运营人员可以在管理端把 Tool / Skill / Knowledge 编排成 Agent，并通过 MCP / A2A / Gateway 对外开放。

![首页](docs/系统截图/首页.png)

## 项目一句话

> **EAF = AI Registry Center + Agent Studio + Tool/Skill Runtime + Governance + Open Protocols for Java enterprises.**

企业 AI 真正难的不是“让大模型回复一句话”，而是：

- 如何让旧系统不重写，也能被 Agent 安全调用。
- 如何让业务系统启动后自动把接口、实例、能力元数据注册到平台。
- 如何避免多项目同名接口、同名 Tool、同名 Skill 互相污染。
...
| **Agent Studio** | 在管理端拖拽 Tool / Skill / Knowledge，调试、发布、灰度、回滚 Agent。 |
| **Skill 三态能力层** | 支持 SubAgentSkill、InteractiveFormSkill、AugmentedTool，把稳定流程封装为可治理能力。 |
| **企业治理护栏** | Tool ACL、sideEffect 分级、IRREVERSIBLE 闸口、Redis 限流、Preflight、Trace Center 与调用审计。 |
| **MCP / A2A / Gateway** | 将受控 Tool、Skill、Agent 暴露给 IDE、外部 Agent 平台、远程 Agent 和业务系统。 |
...
业务系统引入 EAF Starter
  ...
  -> 平台生成字段级 diff
  -> 人工评审 apply / ignore
  -> Agent / Skill / MCP / A2A / Gateway 复用能力
```
...
flowchart LR
  ...
  review --> catalog["Tool / Skill Catalog"]
...
- 管理端菜单已按“概览、AI 注册中心、Agent、Skill、Tool、知识、模型、对外开放、治理”重新组织。
...
sequenceDiagram
  ...
  Studio->>Catalog: select Tool / Skill by qualifiedName
...
| `ai-skill-sdk` | Tool / Skill 开发契约，包含 `AiTool`、`AiSkill`、`ToolRegistry` 和治理元数据。 | - |
...
| `ai-agent-service` | Agent 编排、AI 注册中心、动态 Tool、Skill、Agent Studio、MCP/A2A、Gateway、ACL、Trace 和治理护栏。 | 8603 |
| `ai-admin-front` | Vue 3 管理端，提供注册中心、Agent、Skill、Tool、知识、模型、开放协议和治理页面。 | 3000 |
...
| Skill | Skill 管理、Skill Mining、槽位提取器。 |
...
- 项目隔离：Tool、Skill、Agent、ACL、MCP、A2A、Trace 等资产逐步接入 `projectId / projectCode / environment / tenantId`。
- 稳定引用：Agent 开始支持 `CapabilityReference`，新数据优先使用 `qualifiedName / definitionId`。
- Agent Studio：画布编排、链路调试、版本发布、灰度、回滚。
- Tool / Skill Runtime：动态 HTTP Tool、SubAgentSkill、InteractiveFormSkill、SlotExtractor、Tool Retrieval。
...
- Agent / Skill Market：上架、审批、依赖检查、导出包的数据模型与 API 骨架。
...
| 智能业务助手 | 基于 RAG + Tool + Skill + Agent Studio 构建业务办理 Agent。 |
...
| [AI 中台生态扩展规划](docs/AI中台生态扩展规划.md) | Gateway、MCP、A2A、CLI、Agent / Skill 市场规划。 |
| [产品演进路线](docs/产品演进路线-Skill-AgentStudio-护栏.md) | Skill、Agent Studio、护栏与 Trace 的阶段路线。 |
