...
这不是简单的聊天窗口，也不是把 Workflow Studio 的调试台搬到业务系统。它是一套完整的企业级运行链路：
...
业务用户在业务系统页面中直接使用智能体，不需要跳转到 Workflow Studio，不需要理解工作流节点，也不需要复制粘贴业务数据。用户可以在当前页面询问：
...
嵌入式对话在当前五服务拓扑中的职责划分如下：

- `reachai-control-service`：保持 `/api/embed/**` 公开入口、token exchange、session、message、page action 和管理端 BFF。
- `reachai-runtime-service`：承接 Agent / Workflow / GraphSpec 执行、运行时 Trace、RunOps 和调试链路。
- `reachai-capability-service`：校验 SDK registry credential、origin、agent policy 和 Capability Catalog 相关授权。
- `reachai-knowledge-service`：在嵌入式对话需要知识检索或 RAG 时提供 Knowledge / Retrieval 能力。
- `reachai-model-service`：为 Runtime 或 Knowledge 提供 Chat、Embedding、Rerank 和模型实例能力。

前端和业务系统只依赖 Control 暴露的公开嵌入接口，不直接连接 Runtime 或 Capability 内部端口。已迁入 Control 的路径必须由 owning service 本地实现或通过明确的服务间客户端调用，不再把旧大后端作为隐式兜底。
...
- 平台嵌入式后端公开入口由 `reachai-control-service` 保持 `/api/embed/**` 兼容收口。
  - 当前已迁入 Control：`POST /api/embed/token/exchange`，经 Capability 内部接口校验 registry credential、origin 和 agent policy，再由 Control 签发短期 `embedToken`。
  - 当前已迁入 Control：`POST /api/embed/chat/sessions`，校验 `embedToken` 后创建 `eaf_embed_session`。
  - 当前已迁入 Control：`POST /api/embed/chat/sessions/{sessionId}/messages`，校验 `embedToken + session` 后记录 `eaf_embed_chat_event` 并调用 Runtime agent execution。
  - 当前已迁入 Control：`POST /api/embed/chat/sessions/{sessionId}/messages/stream`，复用 Runtime-backed message 执行链路并输出 `message.delta` / `message.completed` SSE 事件。
  - 当前已迁入 Control：`GET /api/embed/chat/sessions/{sessionId}/page-actions/pending` 与 `POST /api/embed/chat/sessions/{sessionId}/page-actions/{requestId}/result`，读写 Control-owned `eaf_page_action_event`。
  - 剩余 Embed route 必须迁入 `reachai-control-service` 的本地实现，或作为明确兼容桥登记后正式删除。
...
身份与授权模型已经在 `docs/reference/平台身份与授权模型.md` 中定义：
...
| 身份 | 代表 | 凭证 |
| --- | --- | --- |
| 平台管理用户 | 使用 Workflow Studio 和管理端的人 | 平台登录 token，生产接 IAM，本地走 LOCAL |
...
业务系统浏览器登录 cookie、localStorage token、从 DevTools 抓取的临时 header 都不能写入 Workflow Studio、GraphSpec 或 `agent_workflow_credential`，也不能作为生产工具调用凭证。
...
业务用户目录沿用 `docs/reference/平台身份与授权模型.md` 的模型：
...
Workflow Studio 和平台设置中需要提供以下管理能力：
...
## 与 Workflow Studio 的关系

Workflow Studio 是设计、配置、调试和发布工具，不是最终业务用户使用智能体的主入口。

两者关系如下：

| 能力 | Workflow Studio | 业务系统嵌入式对话 |
| --- | --- | --- |
...
Workflow Studio 调试台可以展示 Page Action 请求，但它不能代表真实业务页面执行动作。真实页面动作只能由嵌入了 Chat Widget 和 Page Bridge 的业务页面执行。
