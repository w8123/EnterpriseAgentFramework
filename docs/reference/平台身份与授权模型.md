...
| --- | --- | --- | --- |
| 平台管理员/设计者 | 使用管理端与 Workflow Studio 的人 | 配置模型、工具、Agent、发布流程、查看审计 | 统一 Auth Provider，生产接 IAM，本地开发走 LOCAL 登录 |
...
平台管理用户和业务终端用户必须分开建模。平台管理用户用于登录管理端、Workflow Studio 和管理平台资产；业务终端用户来自业务系统，不要求在平台里维护密码，也不默认具备登录 Workflow Studio 的能力。
...
- Workflow Studio、项目注册管理、模型配置、凭证管理、审计查看等管理端操作统一走平台管理用户和平台 RBAC。
...
平台管理端登录成功后，平台签发自己的管理端访问 token 或 session。这个 token 只用于 Workflow Studio 和管理端 API，不用于业务页面嵌入对话。
...
平台管理端需要独立的用户、角色、权限和登录会话表。它们服务 Workflow Studio 和管理端 API，不服务业务页面里的终端用户登录。
...
`eaf_business_user` 是平台可识别的业务终端用户主体。它不保存密码，不用于登录 Workflow Studio。
...
Workflow Studio 是设计和调试入口，不是最终业务用户的运行入口。
...
- 不要把 Workflow Studio 调试台当成最终用户运行入口。
