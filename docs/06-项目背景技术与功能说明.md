...
| 模块 | 职责 |
| --- | --- |
| `ai-skill-sdk` | 提供业务系统声明 AI 能力的 SDK 契约，包括 `@AiCapability`、`@AiParam`、`@AiOutput`、`@AiTool`、`@AiInteraction` 等注解与交互规范。 |
| `ai-spring-boot-starter` | 面向 Spring Boot 业务系统的接入 Starter，负责能力扫描、注册、心跳、能力同步、SDK 图同步和嵌入式 Runtime 接入。 |
...
- 主动接入：业务系统引入 `ai-spring-boot-starter`，通过注解和启动时扫描主动注册项目、实例、能力和 Agent 图。
