...
flowchart LR
  ...
  catalog --> studio["Workflow Studio 编排"]
...
- Pinia。
- Vue Router。
- Vue Flow 与 AntV G6，用于 Workflow Studio、图谱和画布类能力。
...
| `ai-model-service` | 当前 Model Gateway 部署单元，承载模型实例管理、Chat、Embedding、Rerank 与 OpenAI 兼容代理。 |
...
### 5.3 Agent 管理、Workflow Studio 与 GraphSpec

Agent 侧已经具备从定义、编排、调试到发布的基础闭环：

- Agent 定义管理：名称、描述、模式、模型实例、Runtime 类型、运行位置、工具引用、知识引用和可见性。
- Workflow Studio：可视化画布、节点配置、工具配置、运行配置和 AI 辅助生成/编辑草稿。
...
- 对 Agent 构建：提供 Workflow Studio、GraphSpec、AI 草稿生成、AI 局部编辑、发布校验和版本管理。
...
- 按 Model Gateway、Knowledge / Retrieval、Capability Catalog、Runtime Host 和 Platform Control 五个逻辑域继续收敛新增代码和文档边界。
