...
> 文档版本：v1.5
> 更新时间：2026-04-15
> 状态：ai-skill-sdk / ai-skill-services 已落地；AgentDefinition 驱动路由已完成；Tool REST API 已实现；scanner-first 开发时工具链 MVP（ai-skill-scanner / Tool Manifest / 模板生成 / generate-skill）已落地；AI 能力 API Gateway 待实施
...
Enterprise Agent Framework 已具备完整的 AI Agent 基础设施：
...
├── ai-skill-scanner/    开发时扫描器（OpenAPI / Controller -> Tool Manifest）     [MVP 已实现]
├── skill-services/      生成的 Skill Service 示例目录                            [已验证]
...
├── ai-skill-scanner/           (已实现 MVP) 项目扫描器 + 代码生成器 ✅
│   └── Swagger/OpenAPI 扫描、Controller 扫描、模板代码生成、CLI
├── skill-services/             (已存在示例生成模块) Skill Service 产物目录 ✅
│   └── skill-ai-text-retrieval/ 示例：由 RetrievalController 生成的技能服务
├── templates/                  (已实现 MVP) Skill Service 代码模板 ✅
│   ├── pom.xml.ftl
│   ├── LegacyProjectClient.java.ftl
│   ├── AiTool.java.ftl
│   └── application.yml.ftl
├── .cursor/
│   └── skills/
│       └── generate-skill/     (已实现 MVP) Cursor Skill - 辅助生成 Skill Service ✅
├── ai-model-service/           (已有) 模型网关
├── ai-text-service/            (已有) RAG引擎
├── ai-agent-service/           (已有) 智能体编排
├── ai-admin-front/             (已有) 统一管理前端：RAG/知识库 + Dashboard + Agent 配置/调试（含 AI 能力中台配置）+ 模型调试 + Tool 管理
│
├── deploy/
└── docs/
```
...
### 3.4 ai-skill-scanner — 项目扫描器 ✅ MVP 已实现

**定位**：开发时 CLI 工具，扫描历史项目、输出 `Tool Manifest`，并生成 Skill Service 脚手架。
...
- `scan-openapi`：OpenAPI / Swagger 文档 -> `Tool Manifest`
- `scan-controller`：Spring MVC Controller -> `Tool Manifest`
- `generate`：`Tool Manifest` + Freemarker 模板 -> 可编译 Skill Service 骨架
- `docs/ToolManifest契约.md`：scanner 与生成器共享契约
- `docs/generated-manifests/ai-text-retrieval.yaml`：基于真实控制器生成的 manifest 示例

扫描输出统一的 **Tool Manifest**（JSON/YAML），当前字段格式示例：
...
- 第一批仅覆盖 HTTP 接口型 Tool，参数来源固定为 `PATH / QUERY / BODY`
- OpenAPI 的 header / cookie 参数、`components.parameters` 引用、以及 Service / JavaDoc 深扫仍在后续阶段
- 生成模块当前先打通 `ToolRegistry` 路径；如需完整 ReAct / AgentScope 自动桥接，仍需补 `ToolRegistryAdapter`
...
- 当前模板默认生成 `LegacyProjectClient`，并使用 `RestClient` 发起 HTTP 调用；手写 Skill 仍可按需要使用 Feign

### 3.6 Cursor Skill — AI 辅助生成 ✅ MVP 已实现

当前仓库已提供项目级 Cursor Skill（`.cursor/skills/generate-skill/SKILL.md`），指导 Cursor AI：

1. 读取老项目的目录结构、Controller、Service、Swagger 文档
2. 理解 `Tool Manifest` 契约与 Skill Service 模板
3. 根据老项目特征，生成或校验 Skill Service 脚手架代码
4. 提醒开发者检查 `ToolRegistryAdapter` 等当前仍需人工补齐的运行时桥接点
...
### ~~第 1.5 阶段 — AI 能力中台基础~~ ✅ 已完成

- [x] AgentDefinition 驱动路由 — AgentRouter/AgentFactory 全配置化，消除硬编码 switch-case
- [x] IntentService 动态化 — 意图候选列表从 AgentDefinition 动态生成
- [x] AgentDefinition 扩展字段 — triggerMode、knowledgeBaseGroupId、promptTemplateId、outputSchemaType、useMultiAgentModel
- [x] Tool 管理 REST API — ToolController 暴露 `GET/POST /api/tools`
- [x] 管理前端 AI 能力中台配置 — Agent 编辑页新增配置卡片，列表页新增多维筛选

### ~~第二阶段 — 扫描与生成~~ ✅ MVP 已落地

- [x] 实现 Swagger/OpenAPI 扫描器
- [x] 实现 Controller 注解扫描器（基于 JavaParser）
- [x] 实现 Freemarker 模板代码生成器
- [x] 编写 Cursor Skill（generate-skill），建立 AI 辅助生成流程
- [x] 以 `ai-text-service` 的 `RetrievalController` 验证端到端流程：扫描 → manifest → 生成 Skill Service → `ToolRegistry` 集成

### 第三阶段 — 完善与扩展（持续迭代）

- [ ] AI 能力 API Gateway — `/api/ai/*` 标准化 REST API
- [ ] 结构化输出 — TypedAgentResult + JSON Schema 约束
- [ ] Prompt 模板管理 — PromptTemplate 实体 + CRUD + 前端
- [ ] 多知识库协同检索 — KnowledgeBaseGroup + 多 Collection 并行
- [ ] ToolMetadata 增强（权限要求、来源标识）
- [ ] RemoteTool 基类（HTTP 桥接通用实现）
- [ ] 源码级扫描器（Service 层 + JavaDoc）
- [ ] Python Tool Protocol 支持（RemoteToolProvider）
- [x] Tool 管理前端 — 列表、Schema 展开、测试弹窗（已对接后端 REST）
- [ ] Tool 权限、限流、审计等企业级特性
- [ ] WorkflowEngine 有状态流程编排 + 人机协同
...
AgentDefinition 驱动路由已经实现了"配置即路由"的基础能力。随着 Tool 数量增长，下一步需要演进到：

- **动态 Tool 选择**：Agent 根据用户意图和可用 Tool 列表，自主决策调用哪些 Tool（当前 AgentDefinition 已支持 Tool 集合配置，未来可进一步智能化）
...
## 八、从 Agent 对话平台到企业 AI 能力中台

> **详细方案见独立文档：[企业AI能力中台 — 架构演进方案](企业AI能力中台%20—%20架构演进方案.md)**
>
> 本章为概要说明，完整的差距分析、目标架构、核心概念设计、改造方案和实施路线图请参阅上述文档。

### 8.1 问题的发现

在推进真实业务系统 AI 改造的过程中（以合同管理系统为例），我们发现：**真实的业务 AI 需求远比"聊天机器人"复杂得多。** 业务系统需要 AI 能力深度嵌入到各个环节 — 编辑器里的"AI 补全"、审批流程中的"自动审查"、后台定时任务的"自动预警"。

归纳后，企业需要的是 **7 种标准化 AI 原子能力**：文本生成、内容审查、信息抽取、语义检索、摘要生成、知识问答、数据查询。这些能力是通用的，业务场景是特殊的。

### 8.2 重新定位

> Enterprise Agent Framework 的定位，从"AI Agent 对话平台"演进为"企业 AI 能力中台"。
>
> 它不只是一个聊天机器人后台，而是一套标准化的 AI 能力层，可以嵌入到任何业务系统的任何环节中，让业务系统按需调用。

### 8.3 演进方向概要

| 方向 | 说明 | 状态 |
|------|------|------|
| **AgentDefinition 驱动路由** | 消除硬编码 switch-case，实现"配置即路由" | ✅ 已完成 |
| **IntentService 动态化** | 意图候选列表从 AgentDefinition 动态生成 | ✅ 已完成 |
| **管理前端 AI 能力中台配置** | 触发方式、多 Agent 模型、输出 Schema、知识库组、Prompt 模板 | ✅ 已完成 |
| **Tool 管理 REST API** | `GET/POST /api/tools` 列表、详情、测试执行 | ✅ 已完成 |
| **AI 能力 API Gateway** | `/api/ai/*` 标准化 REST API，业务系统直接调用 | ⏳ 下一步 |
| **结构化输出** | TypedAgentResult + JSON Schema 约束 | ⏳ 待实施 |
| **Prompt 模板管理** | 可配置、可版本管理、可在线调试 | ⏳ 待实施 |
| **多知识库协同检索** | KnowledgeBaseGroup + 多 Collection 并行 + 融合排序 | ⏳ 待实施 |
| **WorkflowEngine** | 有状态流程编排 + 人机协同 | 📋 规划中 |

### 8.4 双向能力流通

```
                    Skill SDK（业务能力 → Agent）
                    ┌──────────────────────┐
                    │                      │
  ┌──────────┐     │   ┌──────────────┐   │     ┌──────────┐
  │ 业务系统  │────▶│   │  AI 能力中台  │   │◀────│ 业务系统  │
  ...
  └──────────┘     │   └──────────────┘   │     └──────────┘
                    │                      │
                    └──────────────────────┘
                    AI 能力 API（AI 能力 → 业务系统）
```

- **Skill SDK** 解决的是"业务系统能力 → Agent"的问题（Agent 调用业务 API）
- **AI 能力 API** 解决的是"AI 能力 → 业务系统"的问题（业务系统调用 AI 能力）
