...
> 文档版本：v1.8
> 更新时间：2026-04-16
> 状态：运行时扫描主线已收口；`ai-skill-services` 与 `ai-skill-scanner` 已下线，scanner 已并入 `ai-skills-service`。
...
├── ai-skills-service/  知识 / Tooling 基础层（RAG + scanner 核心）
...
- `ai-skills-service`：承载 RAG 与 scanner 核心代码，供 `ai-agent-service` 直接复用。
...
- scanner 已迁入 `ai-skills-service` 的 `tooling/scanner` 包。
...
4. ~~在边界稳定后再讨论正式命名与部署迁移~~（已完成：`ai-text-service` → `ai-skills-service`）。

## 六、结论

项目已经完成从“扫描后生成代码”到“扫描后直接入库并运行时注册”的方向收口，同时把 Tooling 能力进一步并入 `ai-skills-service`，让仓库模块边界更简单、主链路更短、维护成本更低。
